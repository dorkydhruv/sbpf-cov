...
fn test_c_example_mollusk_coverage() {
    ...
    let elf_path = manifest_dir.join("target/c_program.so");
    let obj_path = manifest_dir.join("target/c_program.o");
    let program_name = manifest_dir.join("target/c_program");

    if !elf_path.exists() || !obj_path.exists() {
        println!("Skipping test: target/c_program files not compiled yet.");
        return;
    }
    ...
    let trace_path = std::env::temp_dir().join("c_trace.json");
    let trace = ExecutionTrace {
        executed_pcs: pcs.into_iter().collect(),
        program_name: Some("c_program.so".to_string()),
    };
