...
pub fn convert_dump_to_profraw(
    ...
    let dump_content = fs::read_to_string(dump_path)
        .with_context(|| format!("Failed to read dump file {:?}", dump_path))?;
    ...
    let prf_data_bytes = elf_obj
        .section_by_name("__llvm_prf_data")
        .and_then(|s| s.data().ok())
        .unwrap_or(&[]);
    let prf_names_bytes = elf_obj
    ...
    for i in 0..num_data {
        ...
        let nc = u32::from_le_bytes(prf_data_bytes[offset..offset + 4].try_into()?);
        per_fn_num_counters.push(nc);
    }

    let total_counters_from_elf: u64 = per_fn_num_counters.iter().map(|&n| n as u64).sum();

    let program_key = elf_path.file_name().unwrap().to_str().unwrap();
    let counters = dump
        .programs
        .get(program_key)
    ...
    for i in 0..num_data {
        ...
        let new_counter_ptr = cumulative_counter_bytes - (i as i64 * DATA_RECORD_SIZE as i64);
        data_records[rec_offset + COUNTER_PTR_OFFSET..rec_offset + COUNTER_PTR_OFFSET + 8]
            .copy_from_slice(&new_counter_ptr.to_le_bytes());

        data_records[rec_offset + 24..rec_offset + 32].copy_from_slice(&0i64.to_le_bytes());
        data_records[rec_offset + 32..rec_offset + 40].copy_from_slice(&0u64.to_le_bytes());
        data_records[rec_offset + 40..rec_offset + 48].copy_from_slice(&0u64.to_le_bytes());
